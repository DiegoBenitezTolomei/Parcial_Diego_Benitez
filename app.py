...
#esta funcion lee el archivo csv y lo almacena en una lista y que lo retorne
def CsvEnLista():
    with open('clientes.csv', 'r', encoding="utf-8") as clientes:
        leer = csv.reader(clientes)
        listadearchivo= list(leer)
    return listadearchivo
...
def registrar():
    ...
    if formulario.validate_on_submit():
        if formulario.password.data == formulario.password_check.data:
            with open('usuarios', 'a+', newline='') as archivo: #se corrigio el salto de linea al registrarse nuevos usuarios
